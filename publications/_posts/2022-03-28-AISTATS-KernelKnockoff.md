...
# Paper contributions:
- Theortical convergences for new estimators, in particular the general association measure with any bounded kernel, for TR.
...
- $HSIC(X,Y) = \frac{1}{n-1}\text{tr}(KHLH)$ where is the centering matrix $H = I_n - 1_n \mathbf{1}\mathbf{1}^T$ and $K_{ij} = \varphi(x_i, x_j)$ and $L_{ij} = \varphi(y_i, y_j)$
...
2. Non linear data:
$\textbf{2.c : } Y \sim \mathcal{P}(\lambda), \text{ where } \mathcal{P} \text{ is the Poisson distribution where } \lambda = \exp\lbrace \mathbf{1}\mathbf{1}^T$ and $K_{ij} = \varphi(x_i, x_j)$ and _{10}.X_\mathcal{S} \rbrace.$
