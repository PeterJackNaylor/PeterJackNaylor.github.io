...
Prediction of treatment response is one of the most difficult tasks in digital pathology. Unlike tasks like tumour detection and subtyping, where many papers show evidence of high accuracies, there are no known visual cues that would allow for the prediction of treatment response.
...
We believe that this is due to the parameter search space being relatively flat when the number of samples are low, thus early stopping is powerful mean of regularisation and of reproducibility.
